...
const Btn = ({
  ...
  <button
    ...
    onClick={onClick}
    style={{
      padding: xs ? "3px 8px" : sm ? "6px 12px" : "9px 20px",
      borderRadius: 7,
      border: "none",
      cursor: disabled ? "not-allowed" : "pointer",
      fontWeight: 700,
      fontSize: xs ? 10 : sm ? 11 : 13,
...
const doPrint = (title, body) => {
  ...
  w.document
    .write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>AFAY – ${title}</title><style>
...
/* ── Clés de stockage ────────────────────────────────────────────────────── */
const SK = {
  members: "afay:members",
  loans: "afay:loans",
  assist: "afay:assist",
  history: "afay:history",
  fondsAsso: "afay:fondsAsso",
  cotisations: "afay:cotisations",
  dismissed: "afay:dismissed",
  sanctions: "afay:sanctions",
  sorties: "afay:sorties",
};
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (a) setAssist(a);
      if (h) setHistory(h);
      if (f != null) setFondsAsso(f);
      if (c) setCotisations(c);
      if (d) setDismissed(d);
      if (s) setSanctions(s);
      if (so) setSorties(so);
      setReady(true);
  ...
  useEffect(() => {
    if (ready) save(SK.members, members);
  }, [members, ready]);
  useEffect(() => {
    if (ready) save(SK.loans, loans);
  }, [loans, ready]);
  useEffect(() => {
    if (ready) save(SK.assist, assist);
  }, [assist, ready]);
  useEffect(() => {
    if (ready) save(SK.history, history);
  }, [history, ready]);
  useEffect(() => {
    if (ready) save(SK.fondsAsso, fondsAsso);
  }, [fondsAsso, ready]);
  useEffect(() => {
    if (ready) save(SK.cotisations, cotisations);
  }, [cotisations, ready]);
  useEffect(() => {
    if (ready) save(SK.dismissed, dismissed);
  }, [dismissed, ready]);
  useEffect(() => {
    if (ready) save(SK.sanctions, sanctions);
  }, [sanctions, ready]);
  useEffect(() => {
    if (ready) save(SK.sorties, sorties);
  }, [sorties, ready]);

  /* ── Dérivés ────────────────────────────────────────────────────────────── */
  const totFR = useMemo(
    () => members.reduce((s, m) => s + m.fondsRoulement, 0),
    [members]
  );
  const totFC = useMemo(
    () => members.reduce((s, m) => s + m.fondsCaisse, 0),
    [members]
  );
  const actLoans = useMemo(
    () => loans.filter((l) => l.status === "actif"),
    [loans]
  );
  const totDette = useMemo(
    () => actLoans.reduce((s, l) => s + l.montantDu, 0),
    [actLoans]
  );
  const totGains = useMemo(
    () =>
      loans
        .filter((l) => l.status === "rembourse")
        .reduce((s, l) => s + l.interets, 0),
    [loans]
  );

  const membersX = useMemo(
    () =>
      members.map((m) => ({
        ...m,
        prorata: totFR > 0 ? (m.fondsRoulement / totFR) * 100 : 0,
        aJour: m.fondsCaisse >= SEUIL,
        droitPret: !assist.some(
          (a) =>
            a.assistantId === m.id &&
            loans.some((l) => l.id === a.loanId && l.status === "actif")
        ),
  ...
  const alerts = useMemo(() => {
    ...
    actLoans.forEach((l) => {
      const d = dLeft(l.echeance),
        m = members.find((x) => x.id === l.emprunteurId);
      if (d <= 30)
        r.push({
          key: `loan-${l.id}`,
          t: d <= 0 ? "danger" : "warn",
          msg: `${m?.name} — ${
            d <= 0 ? `RETARD ${Math.abs(d)}j` : `Échéance dans ${d}j`
          } (${l.echeance})`,
          amt: fmt(l.montantDu),
        });
    });
    members
      .filter((m) => m.fondsCaisse < SEUIL)
      .forEach((m) =>
        r.push({
          key: `caisse-${m.id}`,
          t: "info",
          msg: `${m.name} — Fonds de Caisse insuffisant (${fmt(
            m.fondsCaisse
          )} / ${fmt(SEUIL)} requis)`,
        })
      );
      ...
      .forEach((s) => {
        ...
        if (m)
          r.push({
            key: `sanction-${s.id}`,
            t: "warn",
            msg: `⚠️ Sanction en attente — ${m.name} : ${s.raison} (${fmt(
              s.montant
            )})`,
          });
      });
  ...
  const visibleAlerts = useMemo(
    () => alerts.filter((a) => !dismissed.includes(a.key)),
    [alerts, dismissed]
  );
  ...
  const openSub = (type, data = {}) => {
    setParentModal(modal);
    setModal({ type, data });
    setForm({ date: tod(), ...data });
  };
  const openM = (type, data = {}) => {
    setParentModal(null);
    setModal({ type, data });
    setForm({ date: tod(), ...data });
  };
  const closeM = () => {
    if (parentModal) {
      ...
      setForm({ date: tod(), ...parentModal.data });
      setParentModal(null);
    } else {
      setModal(null);
      setForm({});
  ...
  const doDeleteMember = (id) => {
    ...
    if (loans.some((l) => l.emprunteurId === id && l.status === "actif")) {
      alert(`Impossible — ${mb?.name} a un prêt actif.`);
      return;
    }
    if (!window.confirm(`Supprimer ${mb?.name} et toutes ses données ?`))
      return;
  ...
  const doAddCotisation = () => {
    const mid = parseInt(form.memberId),
      montant = parseFloat(form.montant),
      date = form.date || tod();
    const mb = members.find((m) => m.id === mid);
    if (!mb || !form.type || !montant || montant <= 0) {
    ...
    setMembers((ms) =>
      ms.map((m) =>
        m.id !== mid
          ? m
          : form.type === "caisse"
          ? { ...m, fondsCaisse: m.fondsCaisse + montant }
          : { ...m, fondsRoulement: m.fondsRoulement + montant }
      )
    );
    setHistory((h) => [
      {
        ...
        heure: new Date().toLocaleTimeString("fr-FR"),
        action: `Cotisation ${
          form.type === "caisse" ? "Fonds de Caisse" : "Fonds de Roulement"
        }`,
        detail: mb.name,
        montant,
      },
  ...
  const doEditCotisation = () => {
    const cotId = parseInt(form.cotisId),
      old = cotisations.find((c) => c.id === cotId);
    if (!old) return;
    if (old.auto) {
      alert("Entrée générée automatiquement — non modifiable.");
      closeM();
      return;
    }
    ...
    const dc =
      (newT === "caisse" ? newM : 0) -
      (old.type === "caisse" ? old.montant : 0);
    const dr =
      (newT === "roulement" ? newM : 0) -
      (old.type === "roulement" ? old.montant : 0);
    setMembers((ms) =>
      ms.map((m) =>
        m.id === old.memberId
          ? {
              ...m,
              fondsCaisse: m.fondsCaisse + dc,
              fondsRoulement: m.fondsRoulement + dr,
            }
          : m
      )
    );
    setCotisations((cs) =>
      cs.map((c) =>
        c.id === cotId ? { ...c, type: newT, montant: newM, date: newD } : c
      )
    );
    pushH(
      "Modif. Cotisation",
      `${mb?.name} — ${newT} ${fmt(newM)}`,
      newM,
      newD
    );
    closeM();
  ...
  const doDeleteCotisation = (cotId) => {
    ...
    if (!c) return;
    if (c.auto) {
      alert("Entrée auto — supprimez le prêt associé pour la retirer.");
      return;
    }
    const mb = members.find((m) => m.id === c.memberId);
    if (!window.confirm(`Supprimer ce versement de ${fmt(c.montant)} ?`))
      return;
    setMembers((ms) =>
      ms.map((m) =>
        m.id !== c.memberId
          ...
          : {
              ...m,
              fondsCaisse:
                c.type === "caisse" ? m.fondsCaisse - c.montant : m.fondsCaisse,
              fondsRoulement:
                c.type === "roulement"
                  ? m.fondsRoulement - c.montant
                  : m.fondsRoulement,
            }
      )
    );
  ...
  const doAddLoan = () => {
    try {
      ...
      if (!rawId || rawId === "") {
        alert("Sélectionnez un emprunteur.");
        return;
      }
      ...
      if (!empr) {
        alert(
          `Emprunteur ID "${rawId}" introuvable. Membres : ${members
            .map((m) => m.id)
            .join(",")}`
        );
        return;
      ...
      // 2. Assistant
      if (assistId)
        setAssist((prev) => [...prev, { assistantId: assistId, loanId }]);
      ...
      setMembers((prev) =>
        prev.map((m) => {
          ...
          return c
            ? {
                ...m,
                fondsRoulement: (m.fondsRoulement || 0) - c.montantContrib,
              }
            : m;
        })
      );
      ...
      const cotisDebits = contribs.map((c, i) => ({
        ...
        auto: true,
        loanId,
        label: "Contribution prêt",
        montant: c.montantContrib,
        date,
        heure: now,
      }));
      ...
      pushH(
        "Prêt accordé",
        `${empr.name}${assistMb ? ` (Asst: ${assistMb.name})` : ""}`,
        montant
      );
  ...
  const doEditLoan = () => {
    ...
    if (old.status === "actif") {
      ...
      const restored = members.map((m) => {
        ...
        return c
          ? { ...m, fondsRoulement: m.fondsRoulement + c.montantContrib }
          : m;
      });
      ...
      const newDebits = newContribs
        .filter((c) => c.montantContrib > 0)
        .map((c, i) => ({
          id: uid() + i,
          memberId: c.memberId,
          type: "roulement",
          ...
          auto: true,
          loanId,
          label: "Contribution prêt (modifié)",
          montant: c.montantContrib,
          date: newDate,
          heure: now,
        }));
      setCotisations((cs) => [
        ...cs.filter((c) => !(c.loanId === loanId && c.direction === "debit")),
        ...newDebits,
      ]);
      setLoans((ls) =>
        ls.map((l) =>
          l.id === loanId
            ? {
                ...
                gainMbr: newGM,
                date: newDate,
                echeance: add3M(newDate),
                assistantId: newAssistId,
    ...
    setAssist((as) => {
      const f = as.filter((a) => a.loanId !== loanId);
      return newAssistId ? [...f, { assistantId: newAssistId, loanId }] : f;
    });
  ...
  const doDeleteLoan = (loanId) => {
    ...
    if (
      !window.confirm(
        `Supprimer le prêt ${
          loan.status === "actif" ? "ACTIF ⚠️" : "remboursé"
        } de ${fmt(loan.montant)} — ${empr?.name} ?`
      )
    )
      return;
  ...
  const doRepay = (loanId, repayDate) => {
    ...
    const { montantDu, interets, fraisAsso, gainMbr } = calcMontantActuel(
      loan,
      date
    );
    ...
    if (contribs.length === 0) {
      ...
      setMembers((ms) => {
        ...
        return ms.map((m) => ({
          ...m,
          fondsRoulement:
            m.fondsRoulement + (t > 0 ? (m.fondsRoulement / t) * gainMbr : 0),
        }));
      });
    } else {
      ...
      const retours = contribs.map((c, i) => {
        ...
        return {
          ...
          cot: {
            ...
            auto: true,
            loanId,
            label: `Retour prêt + gains (${
              interets > 0
                ? "+" + Math.round(interets) + " FCFA d'intérêts"
                : ""
            })`,
            montant: retour,
            date,
      ...
      ls.map((l) =>
        l.id === loanId
          ? {
              ...
              status: "rembourse",
              dateRembours: date,
              montantRembourse: montantDu,
              interetsRembourse: interets,
            }
          : l
      )
    );
    ...
    pushH(
      "Remboursement",
      `${empr?.name} — Intérêts: ${fmt(interets)} | Gains mbr: ${fmt(
        gainMbr
      )} | Asso: ${fmt(fraisAsso)}`,
      montantDu,
      date
  ...
  /* ── CRUD Sanctions ─────────────────────────────────────────────────────── */
  const doAddSanction = () => {
    const memberId = parseInt(form.memberId),
      montant = parseFloat(form.montant),
      date = form.date || tod();
  ...
  const doEditSanction = () => {
    const id = parseInt(form.sancId),
      montant = parseFloat(form.montant),
      date = form.date || tod();
    const mb = members.find((m) => m.id === parseInt(form.memberId));
    if (!mb || !montant || montant <= 0 || !form.raison?.trim()) {
      alert("Remplissez tous les champs.");
      return;
    }
    setSanctions((ss) =>
  ...
  const doDeleteSanction = (id) => {
    ...
    if (s.status === "payee") {
      alert("Sanction déjà remboursée — impossible à supprimer.");
      return;
    }
    if (
      !window.confirm(
        `Supprimer la sanction de ${fmt(s.montant)} pour ${mb?.name} ?`
      )
    )
      return;
  ...
  const doPaySanction = (id) => {
    const s = sanctions.find((x) => x.id === id);
    if (!s || s.status === "payee") return;
    const mb = members.find((m) => m.id === s.memberId);
    setSanctions((ss) =>
      ss.map((x) =>
        x.id === id ? { ...x, status: "payee", datePaiement: tod() } : x
      )
    );
    setFondsAsso((f) => f + s.montant);
    pushH(
      "Sanction remboursée",
      `${mb?.name} — ${s.raison} → Fonds Asso +${fmt(s.montant)}`,
      s.montant
    );
    // Retirer l'alerte associée
    setDismissed((d) =>
      d.includes(`sanction-${id}`) ? d : [...d, `sanction-${id}`]
    );
  };
  ...
  const doAddSortie = () => {
    ...
    if (!form.source) {
      alert("Sélectionnez la source (Fonds Caisse ou Fonds Asso).");
      return;
    }
    ...
    if (source === "caisse") {
      ...
      if (selIds.length === 0) {
        alert("Choisissez au moins un membre concerné par cette sortie.");
        return;
      }
      ...
      const sortie = {
        ...
        heure: new Date().toLocaleTimeString("fr-FR"),
        memberIds: selIds
          .map((id) => members.find((m) => `${m.id}` === `${id}`)?.id)
          .filter(Boolean),
        partParMembre,
      ...
      pushH(
        "Sortie Fonds Caisse",
        `${selIds.length} membre(s) — ${fmt(
          partParMembre
        )}/mbr — ${form.raison.trim()}`,
        montant
      );
    } else {
      // Fonds Asso — pas de sélection de membres
      if (montant > fondsAsso) {
        alert(`Solde insuffisant. Fonds Association : ${fmt(fondsAsso)}`);
        return;
      }
  ...
  const doClearData = () => {
    if (!window.confirm("⚠️ Effacer TOUTES les données ? Irréversible."))
      return;
    setMembers([]);
    setLoans([]);
    setAssist([]);
  ...
  const onFileChange = (e) => {
    ...
    r.onload = (ev) => {
      try {
        ...
        if (!raw._afay_version || !Array.isArray(raw.members))
          throw new Error("Fichier non reconnu.");
        const cf = [];
        (raw.members || []).forEach((m) => {
          if (members.find((x) => x.id === m.id))
            cf.push({
              type: "membre",
              imported: m,
              existing: members.find((x) => x.id === m.id),
            });
        });
        (raw.loans || []).forEach((l) => {
          if (loans.find((x) => x.id === l.id))
            cf.push({
              type: "pret",
              imported: l,
              existing: loans.find((x) => x.id === l.id),
            });
        });
        setImportData(raw);
        setImportConf(cf);
        setImportStep(cf.length > 0 ? 2 : 1);
      } catch (err) {
  ...
  const doImportReplace = () => {
    ...
    setMembers(importData.members || []);
    setLoans(importData.loans || []);
    setAssist(importData.assistance || []);
    setHistory(importData.history || []);
    setFondsAsso(importData.fondsAsso || 0);
    setCotisations(importData.cotisations || []);
    setSanctions(importData.sanctions || []);
    setSorties(importData.sorties || []);
    pushH(
      "Import (remplacement)",
      `${(importData.members || []).length} membres`
    );
    setImportData(null);
    setImportConf([]);
    setImportStep(0);
  ...
  const doImportMerge = () => {
    if (!importData) return;
    const exM = new Set(members.map((m) => m.id)),
      exL = new Set(loans.map((l) => l.id));
    const exC = new Set(cotisations.map((c) => c.id)),
      exA = new Set(assist.map((a) => `${a.assistantId}-${a.loanId}`));
    setMembers([
      ...members,
      ...(importData.members || []).filter((m) => !exM.has(m.id)),
    ]);
    setLoans([
      ...loans,
      ...(importData.loans || []).filter((l) => !exL.has(l.id)),
    ]);
    setCotisations([
      ...cotisations,
      ...(importData.cotisations || []).filter((c) => !exC.has(c.id)),
    ]);
    setAssist([
      ...assist,
      ...(importData.assistance || []).filter(
        (a) => !exA.has(`${a.assistantId}-${a.loanId}`)
      ),
    ]);
  ...
  const prtDash = () =>
    doPrint(
      "Tableau de Bord",
      `<div class="sg"><div class="sb"><div class="sv">${fmt(
        totFC
      )}</div><div class="sl">Fonds Caisse</div></div>
    <div class="sb"><div class="sv">${fmt(
      totFR
    )}</div><div class="sl">Fonds Roulement</div></div>
    <div class="sb"><div class="sv">${fmt(
      totDette
    )}</div><div class="sl">Prêts actifs</div></div>
    <div class="sb"><div class="sv">${fmt(
      fondsAsso
    )}</div><div class="sl">Fonds Association</div></div></div>
    <h2>Membres</h2>
    <table><thead><tr><th>Membre</th><th>Ville</th><th>Fonds Caisse</th><th>Fonds Roulement</th><th>Part%</th><th>Statut</th></tr></thead>
    <tbody>${membersX
      .map(
        (m) =>
          `<tr><td>${m.name}</td><td>${m.ville || "—"}</td><td>${fmt(
            m.fondsCaisse
          )}</td><td>${fmt(m.fondsRoulement)}</td><td>${m.prorata.toFixed(
            2
          )}%</td><td>${
            m.aJour
              ? '<span class="ok">À jour</span>'
              : '<span class="ko">Non à jour</span>'
          }</td></tr>`
      )
      .join("")}</tbody></table>`
    );
  ...
  const prtSanctions = () =>
    doPrint(
      "Sanctions",
      `<div class="sg"><div class="sb"><div class="sv">${
        sanctions.length
      }</div><div class="sl">Total</div></div>
    <div class="sb"><div class="sv">${
      sanctions.filter((s) => s.status === "en_attente").length
    }</div><div class="sl">En attente</div></div>
    <div class="sb"><div class="sv">${fmt(
      sanctions
        .filter((s) => s.status === "payee")
        .reduce((s, x) => s + x.montant, 0)
    )}</div><div class="sl">Encaissé</div></div>
    <div class="sb"><div class="sv">${fmt(
      sanctions
        .filter((s) => s.status === "en_attente")
        .reduce((s, x) => s + x.montant, 0)
    )}</div><div class="sl">En attente (total)</div></div></div>
    <table><thead><tr><th>Date</th><th>Membre</th><th>Raison</th><th>Montant</th><th>Statut</th></tr></thead>
    <tbody>${[...sanctions]
      .sort((a, b) => new Date(b.date) - new Date(a.date))
      .map((s) => {
        const mb = members.find((m) => m.id === s.memberId);
        return `<tr><td>${s.date}</td><td>${mb?.name || "?"}</td><td>${
          s.raison
        }</td><td><strong>${fmt(s.montant)}</strong></td><td>${
          s.status === "payee"
            ? '<span class="ok">Payée</span>'
            : '<span class="wa">En attente</span>'
        }</td></tr>`;
      })
      .join("")}</tbody></table>`
    );
  ...
  const prtSorties = () =>
    ...
    <tbody>${[...sorties]
      ...
      .map(
        (s) =>
          `<tr><td>${s.date}</td><td>${
            s.source === "caisse" ? "Fonds Caisse" : "Fonds Association"
          }</td><td>${s.raison}</td><td><strong>${fmt(
            s.montant
          )}</strong></td></tr>`
      )
      .join("")}</tbody></table>`
  ...
  const prtMember = (mb) => {
    const cots = [...cotisations]
      .filter((c) => c.memberId === mb.id)
      .sort((a, b) => new Date(b.date) - new Date(a.date));
    const lns = [...loans]
      .filter((l) => l.emprunteurId === mb.id)
      .sort((a, b) => new Date(b.date) - new Date(a.date));
    const mX = membersX.find((m) => m.id === mb.id) || mb;
    const totC = cots
      .filter((c) => c.type === "caisse" && !c.auto)
      .reduce((s, c) => s + c.montant, 0);
    const totR = cots
      .filter((c) => c.type === "roulement" && !c.auto)
      .reduce((s, c) => s + c.montant, 0);
    doPrint(
      `Fiche — ${mb.name}`,
      `<table><tbody>
        <tr><td><strong>Nom</strong></td><td>${
          mb.name
        }</td><td><strong>Ville</strong></td><td>${mb.ville || "—"}</td></tr>
        <tr><td><strong>Téléphone</strong></td><td>${
          mb.telephone || "—"
        }</td><td><strong>Part prorata</strong></td><td>${
        mX.prorata?.toFixed(2) || "0.00"
      }%</td></tr>
        <tr><td><strong>Fonds Caisse</strong></td><td>${fmt(
          mb.fondsCaisse
        )}</td><td><strong>Fonds Roulement</strong></td><td>${fmt(
        mb.fondsRoulement
      )}</td></tr>
      </tbody></table>
      <h2>Fonds de Caisse — ${
        cots.filter((c) => c.type === "caisse" && !c.auto).length
      } versements — Total : ${fmt(totC)}</h2>
      ...
      <tbody>${
        cots
          ...
          .map(
            (c) =>
              `<tr><td>${c.date}</td><td>${c.heure || "—"}</td><td>${fmt(
                c.montant
              )}</td></tr>`
          )
          .join("") || `<tr><td colspan="3" style="color:#999">Aucun</td></tr>`
      }</tbody></table>
      <h2>Fonds de Roulement — ${
        cots.filter((c) => c.type === "roulement" && !c.auto).length
      } versements — Total : ${fmt(totR)}</h2>
      <table><thead><tr><th>Date</th><th>Heure</th><th>Montant</th></tr></thead>
      <tbody>${
        cots
          .filter((c) => c.type === "roulement" && !c.auto)
      ...
      <tbody>${
        cots
          ...
          .map(
            (c) =>
              `<tr><td>${c.date}</td><td>${
                c.direction === "debit" ? "📤 Contribution" : "📥 Retour"
              } — ${c.label || ""}</td><td>${
                c.direction === "debit" ? "−" : "+"
              }${fmt(c.montant)}</td></tr>`
          )
      ...
      <tbody>${
        lns
          .map(
            (l) =>
              `<tr><td>${l.date}</td><td>${fmt(l.montant)}</td><td>${fmt(
                l.montantDu
              )}</td><td>${l.echeance}</td><td>${
                l.status === "actif"
                  ? '<span class="wa">En cours</span>'
                  : `<span class="ok">Remboursé ${l.dateRembours || ""}</span>`
              }</td></tr>`
          )
          .join("") || `<tr><td colspan="5" style="color:#999">Aucun</td></tr>`
      }</tbody></table>`
    );
  };
  ...
  const SHdr = ({ title, printFn, onAdd, addLabel }) => (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {printFn && (
          <Btn bg="rgba(255,255,255,0.07)" onClick={printFn}>
            🖨️ Rapport
          </Btn>
        )}
        {onAdd && (
          <Btn bg={C.accent} onClick={onAdd}>
            {addLabel}
          </Btn>
        )}
      </div>
    ...
    >
      ...
      >
          ...
          {TABS.map((t) => (
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                fontSize: 11,
                background: tab === t.id ? C.accent : "rgba(255,255,255,0.05)",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <Btn bg={C.danger} sm onClick={doClearData}>
            🗑️
          </Btn>
          <div
            style={{
      ...
      <div style={{ padding: "20px 24px" }}>
                ...
                {editFA ? (
                  <div style={{ display: "flex", gap: 7 }}>
                    ...
                    <Btn
                      bg={C.green}
                      onClick={() => {
                        const v = parseFloat(editFAV);
                        if (!isNaN(v) && v >= 0) {
                          pushH("Modif. Fonds Asso", "", v);
                          setFondsAsso(v);
                        }
                        setEditFA(false);
                      }}
                    ...
                    <Btn bg={C.muted} onClick={() => setEditFA(false)}>
                      ✕
                    </Btn>
                  </div>
                ) : (
                  ...
                  <tbody>
                    {membersX.map((m) => (
                      <tr key={m.id}>
                        ...
                        <TD s={{ color: C.muted, fontSize: 12 }}>
                          {m.ville || "—"}
                        </TD>
                  ...
                  <tbody>
                    {membersX.map((m) => (
                      <tr key={m.id}>
                        ...
                        <TD>
                          <Badge color={m.aJour ? C.green : C.danger}>
                            {m.aJour ? "✓ À jour" : "✗ En retard"}
                          </Badge>
                        </TD>
                        <TD>
                          {m.loanActif ? (
                            <span style={{ color: C.warn, fontWeight: 600 }}>
                              {fmt(m.loanActif.montantDu)}
                            </span>
                          ) : (
                        ...
                        <TD>
                          ...
                          >
                            <Btn
                              ...
                              sm
                              onClick={() =>
                                openM("addCotisation", { memberId: m.id })
                              }
                            >
                              + Cotis.
                            </Btn>
                            <Btn
                              ...
                              sm
                              onClick={() =>
                                openM("addLoan", { emprunteurId: `${m.id}` })
                              }
                            >
                              + Prêt
                            </Btn>
                            <Btn
                              ...
                              sm
                              onClick={() =>
                                openM("memberDetail", { memberId: m.id })
                              }
                            >
                              👁️
                            </Btn>
        ...
        {tab === "prets" && (
          <div>
            ...
            >
              {[
                { l: "Actifs", v: actLoans.length, c: C.warn },
                {
                  l: "Remboursés",
                  v: loans.filter((l) => l.status === "rembourse").length,
                  c: C.green,
                },
                {
                  l: "Total Intérêts",
                  v: fmt(loans.reduce((s, l) => s + l.interets, 0)),
                  c: C.gold,
                },
              ].map((s, i) => (
                <div
                    ...
                    {loans.map((l) => {
                      ...
                      const actual =
                        l.status === "actif"
                          ...
                          : {
                              montantDu: l.montantRembourse || l.montantDu,
                              interets: l.interetsRembourse || l.interets,
                              periods: "—",
                            };
                        ...
                        >
                          ...
                          <TD>
                            <span
                              style={{
                                fontWeight: 700,
                                color: overdue ? C.danger : C.gold,
                                fontSize: 13,
                              }}
                            >
                              {typeof actual.periods === "number"
                                ? `Période ${actual.periods}`
                                : actual.periods}
                            </span>
                            {overdue && (
                            ...
                            {l.status === "actif" && !overdue && (
                              <div
                                style={{
                                  fontSize: 10,
                                  color: d <= 30 ? C.warn : C.muted,
                                }}
                              >
                                {d}j restants
                              </div>
        ...
        {tab === "cotisations" && (
          <div>
            <SHdr
              title="💰 Cotisations"
              onAdd={() => openM("addCotisation")}
              addLabel="+ Nouvelle Cotisation"
            />
            <div
              style={{
                display: "flex",
            ...
            >
              {[
                { id: "tous", label: "🗂️ Vue d'ensemble" },
                { id: "caisse", label: "🏦 Fonds de Caisse" },
                { id: "roulement", label: "🔄 Fonds de Roulement" },
              ].map((t) => (
                <button
                  key={t.id}
                  onClick={() => setCotisSubTab(t.id)}
                  style={{
                    padding: "7px 16px",
                    borderRadius: 8,
                    border: "none",
                    ...
                    fontWeight: 600,
                    fontSize: 12,
                    background: cotisSubTab === t.id ? C.accent : "transparent",
                    color: cotisSubTab === t.id ? "#fff" : C.muted,
                  }}
                >
                  {t.label}
                </button>
            ...
            {members.length === 0 ? (
              <Card style={{ textAlign: "center", padding: "40px" }}>
                <div style={{ color: C.muted }}>Aucun membre.</div>
              </Card>
            ) : (
              <div>
                {cotisSubTab === "tous" && (
                  <Card>
                    <div
                      style={{ fontSize: 12, color: C.muted, marginBottom: 12 }}
                    >
                      Seuil Fonds de Caisse «à jour» :{" "}
                      <strong style={{ color: C.gold }}>{fmt(SEUIL)}</strong>
                    </div>
                    <table
                      style={{
                        width: "100%",
                    ...
                    >
                      ...
                      <tbody>
                        {membersX.map((m) => {
                          const nb = cotisations.filter(
                            (c) => c.memberId === m.id && !c.auto
                          ).length;
                          const pct = Math.min(
                            (m.fondsCaisse / SEUIL) * 100,
                            100
                          );
                          return (
                            <tr key={m.id}>
                                  ...
                                  >
                                    <div
                                      style={{
                                        width: `${pct}%`,
                                        height: "100%",
                                        background: m.aJour ? C.green : C.warn,
                                        borderRadius: 3,
                                      }}
                                    />
                                  </div>
                              ...
                              <TD>
                                <span
                                  style={{ fontWeight: 700, color: C.gold }}
                                >
                                  {m.prorata.toFixed(2)}%
                                </span>
                              </TD>
                  ...
                  <div>
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {membersX.filter((m) => m.aJour).length}/
                          {members.length}
                        </div>
                        <div
                          style={{
                    ...
                    <Card>
                      ...
                      {cotisations.filter((c) => c.type === "caisse" && !c.auto)
                        .length === 0 ? (
                        <div
                          style={{
                            textAlign: "center",
                          ...
                          <tbody>
                            {[...cotisations]
                              ...
                              .map((c) => {
                                ...
                                return (
                                  <tr key={c.id}>
                                    <TD s={{ fontSize: 11, color: C.muted }}>
                                      {c.date}
                                    </TD>
                                    <TD s={{ fontSize: 11, color: C.muted }}>
                                      {c.heure || "—"}
                  ...
                  <div>
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {
                            cotisations.filter((c) => c.type === "roulement")
                              .length
                          }
                        </div>
                        <div
        ...
        {tab === "sanctions" && (
          <div>
            <SHdr
              title="⚠️ Sanctions"
              printFn={prtSanctions}
              onAdd={() => openM("addSanction")}
            ...
            >
              {[
                ...
                {
                  l: "En attente",
                  v: sanctions.filter((s) => s.status === "en_attente").length,
                  c: C.warn,
                },
                {
                  l: "Payées",
                  v: sanctions.filter((s) => s.status === "payee").length,
                  c: C.green,
                },
                {
                  l: "Encaissé → Fonds Asso",
                  v: fmt(
                    sanctions
                      .filter((s) => s.status === "payee")
                      .reduce((s, x) => s + x.montant, 0)
                      ...
                      .map((s) => {
                        ...
                        return (
                          <tr key={s.id}>
                            ...
                            <TD>
                              <strong>{mb?.name || "Membre inconnu"}</strong>
                              {mb?.ville && (
                                <div style={{ fontSize: 11, color: C.muted }}>
                                  {mb.ville}
                                </div>
                            ...
                            <TD style={{ maxWidth: 220 }}>
                              <span style={{ fontSize: 12 }}>{s.raison}</span>
                            </TD>
                            <TD>
                            ...
                            <TD>
                              {s.status === "payee" ? (
                                <div>
                                  <Badge color={C.green}>✓ Payée</Badge>
                                  {s.datePaiement && (
                                    <div
                                      style={{
                                        fontSize: 10,
        ...
        {tab === "sorties" && (
          <div>
            <SHdr
              ...
              printFn={prtSorties}
              onAdd={() =>
                openM("addSortie", { sortieMembers: members.map((m) => m.id) })
              }
              addLabel="+ Nouvelle Sortie"
            />
            <div
              style={{
                display: "grid",
            ...
            >
              {[
                {
                  l: "Total sorties",
                  v: fmt(sorties.reduce((s, x) => s + x.montant, 0)),
                  c: C.danger,
                },
                {
                  l: "Depuis Fonds Caisse",
                  v: fmt(
                    sorties
                      .filter((s) => s.source === "caisse")
                      .reduce((s, x) => s + x.montant, 0)
                  ),
                  c: C.blue,
                },
                    ...
                    {[...sorties]
                      ...
                      .map((s) => (
                        <tr key={s.id}>
                          ...
                          >
                            {s.date}
                            <div style={{ fontSize: 10 }}>{s.heure || ""}</div>
                          </TD>
                          <TD>
                            <Badge
                              color={s.source === "caisse" ? C.blue : C.purple}
                            >
                              {s.source === "caisse" ? "🏦 Caisse" : "🏛️ Asso"}
                            </Badge>
                          </TD>
                            ...
                            {s.source === "caisse" && s.memberIds ? (
                              <div style={{ lineHeight: 1.6 }}>
                                {s.memberIds.map((id) => {
                                  ...
                                  return mb ? (
                                    <div key={id} style={{ color: C.muted }}>
                                      {mb.name}
                                    </div>
                                  ) : null;
                                })}
            ...
            <Card>
              {history.length === 0 ? (
                ...
                >
                  <thead>
                    <tr>
                      {["Date", "Heure", "Action", "Détail", "Montant"].map(
                        (h) => (
                          <TH key={h} ch={h} />
                        )
                      )}
                  ...
                  <tbody>
                    {history.map((h) => (
                      <tr key={h.id}>
                        ...
                        <TD>
                          <Badge color={C.blue}>{h.action}</Badge>
                        </TD>
                        <TD>{h.detail}</TD>
                        <TD>
                          <span
                            style={{
                              fontWeight: 700,
                              color: h.montant > 0 ? C.gold : C.muted,
                            }}
                          >
              ...
              <div>
                ...
                {form.montant && parseFloat(form.montant) > 0 && (
                  ...
                  >
                    <div>
                      📌 Montant :{" "}
                      <strong>{fmt(parseFloat(form.montant))}</strong>
                    </div>
                    <div>
                      💰 Intérêts (10%) :{" "}
                      <strong style={{ color: C.gold }}>
                ...
                <div style={FR}>
                  ...
                  >
                    ...
                    {membersX
                      .filter(
                        (m) => `${m.id}` !== form.emprunteurId && m.droitPret
                      )
                      .map((m) => (
                        <option key={m.id} value={`${m.id}`}>
                          {m.name}
            ...
            {modal.type === "editLoan" && (
              <div>
                ...
                {editLoanObj?.status === "actif" && (
                  ...
                  >
                    ⚠️ Prêt actif — la modification recalcule les contributions
                    et réajuste les fonds.
                  </div>
                )}
                <div style={FR}>
                  <label style={LBL}>Montant (FCFA) *</label>
              ...
              (() => {
                ...
                return loan && actual ? (
                  <div>
                    ...
                    <div
                      style={{ fontSize: 13, color: C.muted, marginBottom: 18 }}
                    >
                    ...
                    <div
                      style={{
                        background: `${
                          actual.periods > 1 ? C.danger : C.gold
                        }10`,
                        border: `1px solid ${
                          actual.periods > 1 ? C.danger : C.gold
                        }30`,
                    ...
                    >
                      ...
                      {actual.periods > 1 && (
                        ...
                        >
                          {Array.from({ length: actual.periods }).map(
                            (_, i) => (
                    ...
                    >
                      ...
                      <Btn
                        bg={C.green}
                        onClick={() => doRepay(loan.id, repDate)}
                      >
              ...
              <div>
                ...
                <div style={FR}>
                  ...
                  <select
                    ...
                    value={form.source || ""}
                    onChange={(e) => {
                      sf("source")(e.target.value);
                    }}
                  >
                    <option value="">— Sélectionner —</option>
                    <option value="caisse">
                      🏦 Fonds de Caisse (total membres : {fmt(totFC)})
                    </option>
                    <option value="asso">
                      🏛️ Fonds de l'Association ({fmt(fondsAsso)})
                    </option>
                  </select>
                </div>
            ...
            {modal.type === "memberDetail" && detailMb && (
              <div>
                ...
                >
                  <div>
                    <div
                      style={{ fontSize: 19, fontWeight: 800, marginBottom: 3 }}
                    >
                      {detailMb.name}
                    </div>
                    <div
                ...
                >
                  {[
                    ...
                    {
                      l: "Prêt actif",
                      v: detailMb.loanActif
                        ? fmt(detailMb.loanActif.montantDu)
                        : "Aucun",
                ...
                <div style={{ padding: "0 24px 24px" }}>
                  <SectionLabel
                    label={`💰 Cotisations — ${
                      detailCotis.filter((c) => !c.auto).length
                    } manuelles + ${
                      detailCotis.filter((c) => c.auto).length
                    } auto`}
                  />
                    ...
                    <div>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {fmt(
                              detailCotisC.reduce((s, c) => s + c.montant, 0)
                            )}
                          </div>
                          <div
                            style={{
                        ...
                        >
                          ...
                          >
                            {fmt(
                              detailCotisR.reduce((s, c) => s + c.montant, 0)
                            )}
                          </div>
                          <div
                            style={{
                          ...
                          {detailCotis.map((c) => {
                            ...
                            return (
                              <tr key={c.id}>
                                ...
                                <TD>
                                  <strong
                                    style={{ color: isD ? C.danger : C.gold }}
                                  >
                                    {isD ? "−" : "+"} {fmt(c.montant)}
                                  </strong>
                                </TD>
                  ...
                  <SectionLabel
                    label={`💳 Prêts — ${detailLoans.length} au total`}
                  />
            ...
            {importStep === 1 && (
              <div>
                ...
                <div style={{ fontSize: 12, color: C.muted, marginBottom: 18 }}>
                  Exporté le : {importData._exported_label || "inconnu"}
                </div>
                <div
                  style={{
                    display: "grid",
                ...
                >
                  {[
                    {
                      l: "Membres",
                      v: (importData.members || []).length,
                      c: C.blue,
                    },
                    {
                      l: "Prêts",
                      v: (importData.loans || []).length,
                      c: C.gold,
                    },
                    {
                      l: "Sanctions",
                      v: (importData.sanctions || []).length,
                      c: C.warn,
                    },
                    {
                      l: "Fonds Asso",
                      v: fmt(importData.fondsAsso || 0),
                      c: C.purple,
                    },
                  ].map((s, i) => (
                    <div
                    ...
                    >
                      <div
                        style={{ fontSize: 15, fontWeight: 800, color: s.c }}
                      >
                        {s.v}
                      </div>
                      <div
                ...
                >
                  ...
                  <Btn bg={C.blue} onClick={doImportMerge}>
                    🔀 Fusionner
                  </Btn>
                  <Btn bg={C.danger} onClick={doImportReplace}>
                    ♻️ Remplacer
                  </Btn>
                </div>
              </div>
                  ...
                  {importConf.map((c, i) => (
                    ...
                    >
                      <div
                        style={{
                          fontWeight: 700,
                          color: c.type === "membre" ? C.blue : C.gold,
                          marginBottom: 5,
                        }}
                      >
                        {c.type === "membre" ? "👤" : "💳"} Conflit
                      </div>
                      <div
                        style={{
                          display: "grid",
